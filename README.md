...
| **整页重做** | 这一页重讲，**可以换版式**、重挑内容 | 候选版式 + 相邻页不重复 + 容量 + 预演构建 |
...
| 预览页 | 是什么 | 微调 | 整页重做 |
|---|---|---|---|
| 1 / 2 | 封面 / 目录（模板页） | 改 `title`/`subtitle`、目录条目 | ✗（模板页没有版式可换） |
...
| **目录条目 / 封面标题** | **必须同时写 `<name>.outline.json`** | 会被重算（`pipeline.py:1529`/`:1535` 无条件从大纲重算 toc、封面标题派生自 `outline['title']`） |
